...
- **GIVEN** billing record and payment attempt creation requests with correlation ids and
  idempotency keys
- **WHEN** the repository persists the records
- **THEN** those values are written to Billing-owned columns and returned in mapped
  domain contracts
