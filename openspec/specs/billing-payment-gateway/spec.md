...
## Purpose
Defines the provider-neutral `PaymentGateway` abstraction and its Mercado Pago adapter, so the Billing Service can charge and query payments through a swappable, opt-in gateway without coupling routes or repositories to a specific provider.
